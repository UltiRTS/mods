...
return {
    ...
    onDamage = function (unitID, unitDefID, unitTeam, damage, paralyzer, weaponID, projectileID, attackerID, attackerDefID, attackerTeam)
      if (weaponID < 0 or paralyzer or damage < 20.0) then
        return
      end
      ...
      local cond = function (candidateUnit)
        return not (candidateUnit == unitID)
      end
      local wd = WeaponDefs[weaponID]
      local range = getClusterRange(attackerID, wd)
      local unitList = getRandomTargetListInRange(x, y, z, range, Spring.GetUnitAllyTeam(attackerID), cond, 3)
      for i=1,#unitList do
